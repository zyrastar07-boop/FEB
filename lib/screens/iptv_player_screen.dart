...
/// gesture support, auto-retry resilience, and first-class Android TV / D-pad support.
...
class _IptvPlayerScreenState extends State<IptvPlayerScreen>
  ...
  // Gesture tracking
  double _volume = 1.0;
  /// Last aspect ratio actually rendered — used to rebuild the video surface
  /// when the controller reports its real size after initialization (live
  /// HLS streams often start with a 0x0 size).
  ...
  void initState() {
    ...
    // Open fullscreen (landscape) by default; the in-controls toggle can
    // switch to portrait and back.
  ...
  void _playerListener() {
    ...
    // Live streams often don't report their size until the first frames.
    // Rebuild once the real aspect ratio is known so the video is never
    // stuck stretched or letterboxed at the 16:9 fallback.
    final aspect = value.aspectRatio;
    if (aspect > 0 &&
        aspect.isFinite &&
        (aspect - _renderedAspect).abs() > 0.001) {
  ...
  void _nextChannel() {
    final idx =
        widget.allChannels.indexWhere((c) => c.id == _currentChannel.id);
        ...
        child: Stack(
          ...
          children: [
            ...
            if (_initialized && !_hasError)
              Center(
                child: AspectRatio(
                  // Guard against streams that report a 0/unknown size
                  // (common for live HLS and the first frames): fall back
                  // to 16:9 instead of a broken infinite ratio.
            ...
            // Error overlay
            if (_hasError && _retryCount >= _maxRetries) _buildErrorOverlay(),
                ...
                FocusableScale(
                  ...
                  child: TextButton.icon(
                    ...
                    style: TextButton.styleFrom(
                      foregroundColor: AppDesignTokens.gold,
                      backgroundColor:
                          AppDesignTokens.gold.withValues(alpha: 0.15),
                    ),
                  ),
                ),
                const SizedBox(width: 12),
