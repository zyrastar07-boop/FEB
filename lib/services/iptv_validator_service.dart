...
class IptvValidatorService {
  ...
  static const Duration _requestTimeout = Duration(milliseconds: 1600);
  ...
  /// [onBatchChecked] also fires per batch with that batch's channels and
  /// their health flags so the UI can render results incrementally instead
  /// of waiting for the whole playlist to finish checking.
  Future<List<IptvChannel>> validateChannels(
    List<IptvChannel> channels, {
    bool filter = true,
    void Function(int checked, int total)? onProgress,
    void Function(List<IptvChannel> batch, List<bool> healthy)? onBatchChecked,
  }) async {
    ...
    for (var i = 0; i < toCheck.length; i += _batchSize) {
      ...
      final batchFlags = batchResults.map((r) => r.isHealthy).toList();
      ...
      onBatchChecked?.call(batch, batchFlags);
  ...
  static Future<bool> _checkSingle(HttpClient client, String url) async {
    try {
      ...
      await response.drain().timeout(const Duration(milliseconds: 400));
