...
class IptvParserService {
  ...
  Future<List<IptvChannel>> fetchAndParse({
    ...
    // Playlist parsing is pure CPU work (regex + line splitting) — run it on
    // a background isolate so a large playlist never janks the UI thread.
    return compute(parseM3uContent, response.body);
  }

  /// Pure parser – works on any M3U string (useful for testing / offline).
  List<IptvChannel> parseM3u(String content) => parseM3uContent(content);
...
/// Top-level M3U parser usable as a [compute] entry point — parses a playlist
/// body into [IptvChannel]s on a background isolate.
List<IptvChannel> parseM3uContent(String content) {
