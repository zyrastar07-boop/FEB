...
class UserLibraryService extends ChangeNotifier {
  ...
  Future<void> hydrate() async {
    ...
    try {
      ...
      // JSON decode + Movie mapping runs on a background isolate so large
      // libraries never block the main thread at startup (inline on web).
      _watchlist
        ..clear()
        ..addAll(await compute(
            _decodeLibraryList, prefs.getString(_kWatchlist) ?? ''));
      _watched
        ..clear()
        ..addAll(await compute(
            _decodeLibraryList, prefs.getString(_kWatched) ?? ''));
      _liked
        ..clear()
        ..addAll(await compute(
            _decodeLibraryList, prefs.getString(_kLiked) ?? ''));
      _myList
        ..clear()
        ..addAll(await compute(
            _decodeLibraryList, prefs.getString(_kMyList) ?? ''));
      notifyListeners();
    } catch (e) {
...
/// Decodes a persisted library JSON string into [Movie]s. Top-level so it
/// can run on a background isolate via [compute] (inline on web).
      ...
      try {
        ...
        try {
          ...
          out.add(Movie(
            ...
            posterPath:
                (map['poster_path'] ?? map['posterPath'] ?? '') as String?,
            ...
            releaseDate: (map['release_date'] ?? map['first_air_date'] ?? '')
                as String,
            mediaType:
                (map['media_type'] ?? map['mediaType'] ?? 'movie') as String,
          ));
        }
      } catch (_) {}
    }
