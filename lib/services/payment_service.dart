...
class PaymentService extends ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    try {
      ...
      // Provider + transaction ID are sensitive — read via SecurePrefs
      // (transparently decrypts values written before encryption was added).
      lastProvider = await SecurePrefs.readString('${_prefsPrefix}provider');
      lastTransactionId =
          await SecurePrefs.readString('${_prefsPrefix}txId');
  ...
  Future<void> _persist() async {
    ...
    if (lastProvider != null) {
      await SecurePrefs.writeString('${_prefsPrefix}provider', lastProvider!);
    }
    if (lastTransactionId != null) {
      await SecurePrefs.writeString('${_prefsPrefix}txId', lastTransactionId!);
    }
  }
